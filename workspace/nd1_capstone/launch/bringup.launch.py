#!/usr/bin/env python3
# ════════════════════════════════════════════════════════════════
#  ND1 캡스톤 — 5노드 일괄 기동 launch (+ 선택적 SLAM/Nav2 포함)
#
...
#
#  복합 목표 테스트(상 난이도):
#     ros2 topic pub --once /goal_command std_msgs/msg/String "data: 'A구역과 C구역의 박스를 B구역으로 모아줘'"
#
#  사용 (실연동 통합 — Gazebo는 터미널1에서 먼저 기동):
#     # 터미널1: ros2 launch turtlebot4_ignition_bringup turtlebot4_ignition.launch.py
#     # 터미널2(이 런처가 SLAM+Nav2+5노드를 한 번에):
...
def generate_launch_description():
    ...
    includes = [
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                PathJoinSubstitution([tb4_nav, "launch", "slam.launch.py"])
            ),
            condition=IfCondition(use_slam),
        ),
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                PathJoinSubstitution([tb4_nav, "launch", "localization.launch.py"])
            ),
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                PathJoinSubstitution([tb4_nav, "launch", "nav2.launch.py"])
            ),
    ...
    nodes = [
        Node(
            package="nd1_capstone",
            executable="node_a_llm",
            name="node_a_llm",
            output="screen",
        ),
        Node(
        ...
        Node(
            package="nd1_capstone",
            executable="coordinator_fsm",
            name="coordinator_fsm",
            output="screen",
            parameters=[
                {
                    "sim_mode": sim,
